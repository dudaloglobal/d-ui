...
29. `Card` / `Badge` / `Avatar` : `Card` statique = `article` ; interactive = vrai `<a href>` ou `<button type="button">`, jamais un `div` cliquable. Pas de contrôle imbriqué dans une carte lien/bouton. `CardTitle` = `h3` par défaut. `Badge` = `span` nommé par son texte (couleur seule insuffisante, 1.4.1) ; icône et point `aria-hidden`. `Avatar` : `alt` / `name` ; fallback initiales ou silhouette toujours nommé ; `AvatarGroup` = `role="group"` + pastille « +N » nommée. Pas de `CourseCard` métier dans le package.
