...
27. `Alert` / `Notification` / `EmptyState` / `ErrorState` : `Alert` = Message LumApps inline (`hasBackground`, `actions`, fermeture `info` + fond). `Notification` = toast LumApps (`role="alert"`, `actionLabel` / `onActionClick`). Variantes `info` / `success` / `warning` / `danger` (LumApps `error`). `EmptyState` : titre `h2`, actions `Button`. `ErrorState` : `live` pour échec dynamique. File toast = DS-033.
