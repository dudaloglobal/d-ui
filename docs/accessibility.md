...
26. `Navbar` / `Sidebar` : `Navbar` est un `<header>` (slots `brand` / `actions` / `user`). Bouton menu nommé (`aria-expanded`, `aria-controls`, `aria-haspopup="dialog"`) seulement si `onMenuOpenChange`. `Sidebar` : `<nav>` nommé + liste, items `<a href>` (ou bouton), page courante `aria-current="page"`. Replié : icône + nom (`VisuallyHidden`) + `Tooltip` au survol / focus. Overlay mobile : `dialog` modal, Escape / fond / fermer. `SkipLink` avant la coquille ; `<main id="main" tabIndex={-1}>`. Pas de sitemap LMS, pas de `usePathname`.
