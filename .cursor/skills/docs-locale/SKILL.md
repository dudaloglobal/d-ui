...
3. **Example labels** in stories go through `packages/ui/.storybook/docs-locale.ts` (`buttonCopy` / `inputCopy` / `timeAgoCopy` / `selectionCopy` / `skipLinkCopy` / `visuallyHiddenCopy` / `themeCopy` / `typographyCopy` / `iconCopy`). Add every new string to **both** `fr` and `en` with the **same keys**.
